...
const TableContext = React.createContext<{ dense?: boolean }>({ dense: false });
...
>(({ className, ...props }, ref) => (
  <thead ref={ref} className={cn("bg-slate-50 dark:bg-slate-900/50 [&_tr]:border-b", className)} {...props} />
));
...
>(({ className, ...props }, ref) => (
  <tr
    ref={ref}
    className={cn(
      "border-b border-slate-200 transition-colors hover:bg-slate-50/50 data-[state=selected]:bg-slate-100 dark:border-slate-800 dark:hover:bg-slate-900/50 dark:data-[state=selected]:bg-slate-800",
      className
    )}
    {...props}
  />
  ...
  return (
    <td
      ...
      className={cn(
        ...
        dense ? "py-1.5 text-xs" : "py-3 text-sm",
        className
      )}
      {...props}
    />
