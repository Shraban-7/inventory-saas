...
use App\Domain\Events\PurchaseOrderCancelled;
...
final class CancelPurchaseOrderAction
{
    public function handle(int $purchaseOrderId, int $actingUserId): PurchaseOrder
    {
        return DB::transaction(function () use ($purchaseOrderId, $actingUserId): PurchaseOrder {
            ...
            if (! in_array($status, [PurchaseOrderStatus::Draft, PurchaseOrderStatus::Confirmed], true)) {
                throw new InvalidPurchaseOrderStateException('Only a draft or confirmed purchase order can be cancelled.');
            }
            ...
            AuditLog::query()->create([
                ...
                'old_values' => ['status' => $status->value],
                'new_values' => ['status' => PurchaseOrderStatus::Cancelled->value],
            ]);

            $id = (int) $purchaseOrder->getKey();
            DB::afterCommit(static fn () => event(new PurchaseOrderCancelled($id)));
